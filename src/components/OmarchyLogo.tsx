...
/**
 * Omarchy's official wordmark, taken verbatim from
 * /usr/share/omarchy/logo.svg (shipped with the OS).
 * MIT, Copyright (c) David Heinemeier Hansson — see NOTICE.
 * Recolour via the `fill` prop or a text-* class with fill="currentColor".
 */
export function OmarchyLogo({
  width = 64,
  className,
  fill = "currentColor",
}: {
  ...
  return (
    <svg
      ...
      className={className}
      fill="none"
      /* Sits the wordmark's own baseline on the text baseline. Transform, not
         margin, so the lockup's height and gap are unaffected. */
      style={{ transform: `translateY(${wordmarkBaselineOffset(height)}px)` }}
    >
      <g fill={fill}>
        <path
          clipRule="evenodd"
